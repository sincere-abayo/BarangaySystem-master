...
$clearances = $certificate->view_clearance();
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    <meta charset="UTF-8">
    <title>Staff Clearance Report</title>
...
<body>
    <div class="container mt-4">
        <h2 class="mb-4">Clearance Report</h2>
        <div class="row filter-row">
            <div class="col-md-3 mb-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search clearances...">
            </div>
            <div class="col-md-3 mb-2">
        ...
        <div class="mb-3">
            <strong>Total Clearances:</strong> <span
                id="totalClearances"><?= is_array($clearances) ? count($clearances) : 0 ?></span>
        </div>
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped" id="clearanceTable">
                ...
                <tbody>
                    <?php if (is_array($clearances))
                        foreach ($clearances as $clr) { ?>
                            <tr>
                                <td><?= str_pad($clr['id_clearance'], 6, '0', STR_PAD_LEFT) ?></td>
                                <td><?= htmlspecialchars($clr['fname'] . ' ' . $clr['mi'] . ' ' . $clr['lname']) ?></td>
                                <td><?= htmlspecialchars($clr['age']) ?></td>
                                <td><?= htmlspecialchars($clr['houseno'] . ', ' . $clr['street'] . ', ' . $clr['brgy'] . ', ' . $clr['municipal']) ?>
                                </td>
                                <td><?= htmlspecialchars($clr['purpose']) ?></td>
                                <td><?= htmlspecialchars($clr['status']) ?></td>
                                <td><?= htmlspecialchars($clr['date'] ?? '') ?></td>
                            </tr>
                        <?php } ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function filterTable() {
            ...
            totalClearances.textContent = count;
        }
        ...
        function exportPDF() {
            ...
            doc.text('Clearance Report', 14, 16);
            doc.autoTable({
                html: '#clearanceTable',
                startY: 22,
