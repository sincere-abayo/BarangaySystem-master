...
echo "<h2>Testing Certificate of Indigency Notification System</h2>";
...
try {
    ...
    // Test with a real indigency ID from your database
    $test_indigency_id = 1; // Use a real indigency ID from your database
    $test_resident_id = 24; // Use a valid resident ID from the database

    echo "<h3>1. Testing Indigency Data Retrieval</h3>";
    ...
    // Test data for indigency notification
    $indigency_data = [
        'service_type' => 'Certificate of Indigency',
        'certificate_number' => 'IND-' . $test_indigency_id,
        'date' => date('Y-m-d'),
    ...
    if ($emailResult && $smsResult && $updateResult) {
        echo "<p style='color: green; font-weight: bold;'>🎉 All tests passed! Certificate of Indigency notification system is working correctly.</p>";
    } else {
