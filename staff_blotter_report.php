...
<head>
    <meta charset="UTF-8">
    <title>Staff Blotter Report</title>
...
<body>
    <div class="container mt-4">
        <h2 class="mb-4">Blotter Report</h2>
        <div class="row filter-row">
            <div class="col-md-3 mb-2">
                <input type="text" id="searchInput" class="form-control" placeholder="Search blotters...">
            </div>
            <div class="col-md-3 mb-2">
        ...
        <div class="mb-3">
            <strong>Total Blotters:</strong> <span
                id="totalBlotters"><?= is_array($blotters) ? count($blotters) : 0 ?></span>
        </div>
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped" id="blotterTable">
                ...
                <tbody>
                    <?php if (is_array($blotters))
                        foreach ($blotters as $b) { ?>
                            <tr>
                                <td><?= str_pad($b['id_blotter'], 6, '0', STR_PAD_LEFT) ?></td>
                                <td><?= htmlspecialchars($b['fname'] . ' ' . $b['mi'] . ' ' . $b['lname']) ?></td>
                                <td><?= htmlspecialchars($b['houseno'] . ', ' . $b['street'] . ', ' . $b['brgy'] . ', ' . $b['municipal']) ?>
                                </td>
                                <td><?= htmlspecialchars($b['contact']) ?></td>
                                <td><?= htmlspecialchars($b['narrative']) ?></td>
    ...
    <script>
        ...
        const totalBlotters = document.getElementById('totalBlotters');
        function filterTable() {
            ...
            totalBlotters.textContent = count;
        }
